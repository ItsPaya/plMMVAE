...
class VAEbimodalCelebA(BaseMMVae, pl.LightningModule):
    ...
    def get_random_style_dists(self, num_samples):
        s1_mu = torch.zeros(num_samples, self.flags.style_img_dim)
        s1_logvar = torch.zeros(num_samples, self.flags.style_img_dim)
        s2_mu = torch.zeros(num_samples, self.flags.style_text_dim)
        s2_logvar = torch.zeros(num_samples, self.flags.style_text_dim)
